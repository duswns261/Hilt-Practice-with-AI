...
앱을 실행하면 먼저 **Home 화면**이 표시됩니다.  
Home 화면에서 `user_123 프로필 보기` 버튼을 누르면 사용자 프로필 화면으로 이동합니다.

1. **Home** — 학습용 진입 화면, 버튼 클릭으로 사용자 상세 이동
2. **로딩** — 데이터를 불러오는 동안 중앙에 스피너 표시
3. **성공** — 사용자 아바타(이름 첫 글자), 이름, ID, 상세 카드 표시
4. **에러** — 전체 화면 중앙에 경고 아이콘과 에러 메시지 표시
   - `네트워크 오류가 발생했습니다.`
   - `사용자를 찾을 수 없습니다.`
   - `알 수 없는 오류가 발생했습니다.`
...
Home 화면에서 버튼을 눌러 사용자 프로필 화면으로 이동할 수 있습니다.  
...
| 6 | 단위 테스트 10개 작성 |
| 7 | Navigation 완성 — `HomeRoute` 추가, `AppNavHost` 연결 |
| 8 | domain 레이어 정리 — `data/model`·`data/repository` 중복 제거 |
| 9 | 미사용 기본 예제 테스트 정리 — Example 테스트 제거 |
...
- `CancellationException` rethrow 처리 (`UserRepositoryImpl`, `UserViewModel`)
